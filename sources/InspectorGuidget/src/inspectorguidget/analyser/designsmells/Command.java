...
public class Command {
	...
	ListenerType		evType;
	SubGraph			graph;
	// List<Action> candidateCommands; //Commands with conditions that manage at
	// least one listener event.
	List<Action>		commandsWithAllConds;	// Commands that manage widgets
												// attributes in their
												// conditional statements.
												// Storing all conditions
												// surrounded by a command, for
												// example, all non executed
												// conditions
	List<Action>		commands;				// Filter commandsWithAllConds
												// by removing non executed
												// conditions of a command
												// (e.g., previous else
												// statements)
	...
	public Command(ListenersWrapper procWrapper, Factory factory) {
		...
		// int totalLOCondListeners =
		// getTotalLoc(getLOCListeners(allCondListeners));
	...
	// Traverse the CFG starting from the entry node and get all elements that
	// are into eventRef conditionals
	...
	private List<CtExpression<?>> looking4Conditionals(List<CtCodeElement> elements) {
		...
		if (!allConditions.isEmpty()) {
			...
			// It means that has at least one condition that manage the event or
			// widget
	...
	private boolean isConditionEventRef(CtExpression<?> condition) {
		...
		for (CtParameter<?> parameter : parameters) {// get parameters'type of a
														// method
			typesParam.add(parameter.getType());
		}

		Set<CtVariable<?>> variables = defuse.getDeepDef(condition);// TODO:
																	// improve
																	// to check
																	// firstly
																	// type of
	...
	// Workaround to force to get definitions for some variables that is not
	// possible get the declaration/defs by using defuse.getDeepDef(condition):
	// bug in defuse
	...
	// Gather elements of commands that are found into same conditional
	// statement
	public List<Action> gatherCommands(List<Action> actions) {
		...
		for (Action action : actions) {
			...
			if (!linked.contains(action)) {
				...
				// cmds.add(new Action(statements, action.getConditions(),
				// action.getSource()));
				// }
	...
	private List<CtExpression<?>> processLine(CtCodeElement line) {
		...
		} else if (line instanceof CtExpression) {// CtInvocation,
													// CtVariableAccess,
													// CtFieldAccess,
													// CtAssignment, etc.
	...
	public List<Action> mergeCommands(List<Action> cmds) {
		...
		for (Action cmd : cmds) {
			...
			for (CtExpression<?> cond : conditions) {
				if (isCondRefWidgets(cond) || isConditionEventRef(cond)) {// Get
																			// only
																			// the
																			// conditions
																			// that
	...
	public List<Action> removeNestedCommands(List<Action> actions) {
		...
		// IdentityHashMap<CtMethod, List<Action>> cmds =
		// gatherCommandsBySource(actions);
		// for (Entry<CtMethod, List<Action>> entry: cmds.entrySet()){
		// IdentityHashMap<Action, List<Action>> cmdsANDNested = new
		// IdentityHashMap<Action, List<Action>>();
		//
		...
		// tmpPosLine > cmdPosLine &&//tmpLine should be greater cmdLine to be
		// nested
		// isNested(tmp.getStatements(), cmd.getStatements())){//tmp is
		// contained by command
		// nested.add(tmp);
		// }
		// }
		// }
		...
		for (Action cmd : actions) {// want to know all nested for this command
			...
			while (it.hasNext()) {
				...
				if (cmd != tmp) {// not compare the same command
					int tmpPosLine = tmp.getStatements().get(0).getPosition().getLine();
					if (cmd.getSource() == tmp.getSource() && tmpPosLine > cmdPosLine && // tmpLine
																							// should
																							// be
																							// greater
																							// cmdLine
																							...
																							// same
																							// stmt
							isNested(tmp.getStatements(), cmd.getStatements())) {// tmp
																					// is
																					// contained
																					// by
																					// command
	...
	// Verify if statements1 is contained into statements2: TODO: Refactor by
	// merging with isContainedBy
	public boolean isNested(List<CtCodeElement> candidate, List<CtCodeElement> cmdStmts) {
		ConditionalListeners cl = new ConditionalListeners();
		boolean isNested = false;
		for (CtCodeElement cand : candidate) {// Check all statements of the
												// candidate to nested
			if (!cl.isConditionalStatement(cand)) {
				for (CtCodeElement stmt : cmdStmts) {
					if (isContainedBy(cand, stmt)) {// Check if the cand is
													// contained into the stmt
						isNested = true;
					} else {
						return false;
					}
	...
	public boolean isAllNested(List<Action> cmds, IdentityHashMap<Action, List<Action>> cmdsANDparents2) {
		...
		for (Action cmd : cmds) {
			...
			if (parents != null) {
				...
				if (parents.containsAll(cmds2)) {// If some cmd has all parents
													// in the list cmds2, it
													// means all cmds are nested
					return true;
				}
			}
		}
	...
	/*
	 * Refine commands by recovering only commands that contain at least one
	 * keyword "instanceof" in their conditionals That means, commands that can
	 * listen to more one widget type
	 */
	...
	// Get commands surrounded by all conditions including non executed
	// conditions
...
// String fileName = clazz.getQualifiedName() + "."+
// entry.getKey().getSimpleName() + ".txt";
...
// cmd.getSource().getDeclaringType().getQualifiedName());
...
// if ( isCondRefWidgets(condition)){//Should verify if the widget condition
// handles the listener event
 ...
 */
// private IdentityHashMap<CtMethod,List<Action>>
// gatherCommandsBySource(List<Action> commands){
// IdentityHashMap<CtMethod, List<Action>> res = new IdentityHashMap<CtMethod,
// List<Action>>();
//
// for (Action cmd : commands){
