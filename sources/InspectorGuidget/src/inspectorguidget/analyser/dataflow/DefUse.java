...
public class DefUse {
	...
	public DefUse(ControlFlowGraph cfg) {
		...
		// cfg.getMethod().getDeclaringType().getQualifiedName());
	...
	private void init() {
		...
		if (parent instanceof CtClass) {
			attributes = ((CtClass<?>) parent).getFields(); // TODO: look at
															// super classes
		}
	}
	...
	/**
	 * Parent is the last visited block and child is yet visited but need to
	...
	/**
	 * Entry point Breadth first exploration of the blocks of the cfg
	 */
	...
	//
	// /**
	// * Get fields from classes hierarchy & containing class if @clazz is
	// nested
	// */
	...
	/**
	 * Two elements should have a different ID if their have the same hashcode
	 * but different location in the source code.
	 */
	private String getUniqueID(CtElement elem) {
		if (elem.getPosition() == null)
			return "GeneratedStatement" + System.currentTimeMillis();
		return elem.getPosition().getFile().getName() + ":" + elem.getPosition().getSourceStart() + ":"
				+ elem.getPosition().getSourceEnd() + ":" + elem.hashCode();
	}
	...
	/**
	 * Get possibles definitions for this variable at this line. Can return null
	 * if not defined (i.e: access to final variables from nested classes)
	 */
	...
	/**
	 * Get attributes & methods parameters dependences from all conditions from
	 * the root block to this block
	 */
	...
	/**
	 * Return fields and parameters accessed in all conditional edges of the
	 * control flow graph
	 */
	...
	/**
	 * Return definitions of block @two that are not in block @one Because
	 * blocks can have severals parents, definitions are union-based
	 */
	...
	/**
	 * Return definitions after the last statement of this block. Because blocks
	 * can have severals parents, definitions are union-based
	 */
	...
	/**
	 * Return conditions using @field (which is not redefined before the
	 * conditional in its method)
	 */
	public List<CtExpression<?>> getConditions(CtVariable<?> field) {
		...
		for (BasicBlock block : cfg.getAllNode()) {
			for (BasicBlock child : block.getChildren()) {
				CtExpression<?> condition = block.getCondition(child);
				if (getDeepDef(condition).contains(field))
					res.add(condition);
			}
		}
		return res;
	...
	public String toString() {
		...
		// for(CtParameter param : params){
		// res.append("\""+param.hashCode()+"\""+"[shape=\"box\"][label=\""+param.toString().replaceAll("\"",
		// "\\\\\"").replaceAll("\n", "\\\\l").replaceAll(";", " ")+"\"]\n");
		// }
		//
		...
		// res.append("\""+field.hashCode()+"\""+"[shape=\"hexagon\"][label=\""+field.toString().replaceAll("\"",
		// "\\\\\"").replaceAll("\n", "\\\\l").replaceAll(";", " ")+"\"]\n");
		// }
		...
		for (BasicBlock block : cfg.getAllNode()) {
			for (CtCodeElement line : block.getElements()) {
				List<CtVariableAccess<?>> vars = getUsedVariables(line);
				res.append("\"" + getUniqueID(line) + "\"" + "[label=\""
						+ line.toString().replaceAll("\"", "\\\\\"").replaceAll("\n", "\\\\l").replaceAll(";", " ")
						+ "\"]\n");
				if (vars != null && !vars.isEmpty()) {
					...
					for (CtVariableAccess<?> var : vars) {
						...
						if (defs == null && decl instanceof CtField) {
							...
							res.append("\"" + getUniqueID(decl) + "\"" + "[shape=\"hexagon\"][label=\""
									+ decl.getSignature() + "\"]\n");
							...
							res.append("\""
									...
									+ "\""
									+ "[shape=\"box\"][label=\""
									+ decl.toString().replaceAll("\"", "\\\\\"").replaceAll("\n", "\\\\l")
											.replaceAll(";", " ") + "\"]\n");
						} else if (defs != null) { // complain here if null &
													// def in the source code?
							for (CtCodeElement def : defs) {
