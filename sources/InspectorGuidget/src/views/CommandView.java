...
public class CommandView extends ViewPart {
	...
	private TableViewer makeTable(Composite parent) {
		...
		tableViewer.setLabelProvider(new LabelProvider() {
			...
			public String getText(Object element) {
				...
				String label = GUICommands.getLabel((IMarker) element);
				if (label != null)
					return label;
		...
		tableViewer.addCheckStateListener(new ICheckStateListener() {
			...
			public void checkStateChanged(CheckStateChangedEvent event) {
				...
				String file = store.getString("pathCommand");

				boolean checked = event.getChecked();

				IMarker marker = (IMarker) event.getElement();
				String info = GUICommands.getInfo(marker);
	...
	public static void addMarker(IMarker marker) {
		...
		getSingleton().setPartName(size + " GUI commands");
